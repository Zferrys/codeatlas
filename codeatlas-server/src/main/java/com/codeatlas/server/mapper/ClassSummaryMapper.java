...
public interface ClassSummaryMapper {
    ...
    @Select("SELECT * FROM class_summary WHERE project_id = #{projectId} AND fqn = #{fqn} ORDER BY scan_id DESC LIMIT 1")
    @Results({
    ...
    ClassSummaryEntity findByProjectIdAndFqn(@Param("projectId") Long projectId, @Param("fqn") String fqn);
